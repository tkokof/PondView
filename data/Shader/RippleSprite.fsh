...
const float thresholdDistance = 0.8;

// calc new ripple uv
vec2 waveUV(vec2 waveCenter, vec2 pos, float time, float strength)
{
    vec2 uv = pos;	
    vec2 toPixel = uv - waveCenter; // vector from center to pixel
    float distance = length(toPixel);
	if (distance > thresholdDistance) return uv;
...
void main()
{
   ...
   for (int i = 0; i < MAX_RIPPLE_SIMUL_COUNT; ++i)
   {
       ...
       vec2 tempUV = waveUV(u_ripple[i], v_texCoord, u_time[i], u_strength[i]);
       uv = (uv + tempUV) * 0.5;
   }
